...
const App: React.FC = () => {
  const [gamePhase, setGamePhaseInternal] = useState<GamePhase>(GamePhase.TITLE);
  ...
  const [battleTransitionState, setBattleTransitionState] = useState<'none' | 'biker_defeated'>('none');
  ...
  const startNewGameFlow = () => {
    ...
    setGamePhaseInternal(GamePhase.NAME_INPUT);
  };

  const continueGameFlow = () => {
    handleStopTitleMusic();
    if (player) {
      setGamePhaseInternal(GamePhase.WORLD_MAP);
    } else {
  ...
  const setGamePhase = useCallback((phase: GamePhase, associatedData?: string) => {
  ...
  useEffect(() => {
    if (gamePhase === GamePhase.BATTLE && currentRun && player) { 
      ...
      const enemies = getEnemiesForEncounter(region, currentRun.currentEncounterIndex, currentRun.currentStageInBossFight);
      const isBoss = enemies.length === 1 && (
        ...
        enemies[0].id === ALL_ENEMIES.e_micchy_dragonlord.id || // Added new boss
  ...
  const initiateRegionRun = (regionId: string) => {
    ...
    setCurrentRun({
      ...
      xpGainedThisRun: 0,
      goldGainedThisRun: 0,
      currentStageInBossFight: (regionId === REGIONS.r_micchy_tower.id && selectedRegion.encounters.length === 0) ? 1 : undefined, // Start at stage 1 for Mitchy Tower boss
    });
  ...
  const handleAttemptRegionSelect = (regionId: string) => {
    ...
    if (regionId === REGIONS.r_micchy_castle.id) {
      ...
      setGamePhase(GamePhase.FINAL_BOSS_PRE_DIALOGUE);
      return;
    }
    ...
    if (isBossFirstRegion) {
      ...
      // This check uses the (now aligned) bossUnlockLevel.
      ...
      setGamePhase(GamePhase.BOSS_CONFIRMATION); 
    } else {
  ...
  const handleBattleEnd = (win: boolean, xpGained: number, goldGained: number) => {
    ...
    const isMitchyTower = currentRun.currentRegionId === REGIONS.r_micchy_tower.id;
    const isBikerBossFight = isMitchyTower && currentRun.currentEncounterIndex === currentRegion.encounters.length && (currentRun.currentStageInBossFight === 1 || !currentRun.currentStageInBossFight);
    const isDragonlordBossFight = isMitchyTower && currentRun.currentEncounterIndex === currentRegion.encounters.length && currentRun.currentStageInBossFight === 2;
    const defeatedBossId = (win && currentRun.currentEncounterIndex === currentRegion.encounters.length) ? 
                           (isDragonlordBossFight ? ALL_ENEMIES.e_micchy_dragonlord.id : currentRegion.bossId) 
                           : null;


    let fragmentsObtainedThisBattle: Item[] = [];

    if (win) {
        // Forest Fragment from Micchy Sexy Knight
        if (defeatedBossId === ALL_ENEMIES.e_micchy_sexy_knight_boss.id && !player.inventory.some(item => item.id === ALL_ITEMS.i_key_fragment_forest.id)) {
            if (Math.random() < 0.5) {
                const fragment = createItemInstance(ALL_ITEMS.i_key_fragment_forest.id);
                if (fragment) fragmentsObtainedThisBattle.push(fragment);
            }
        }
        // Cave Fragment from Micchy Baroku Saburou
        else if (defeatedBossId === ALL_ENEMIES.e_micchy_baroku_saburou_boss.id && !player.inventory.some(item => item.id === ALL_ITEMS.i_key_fragment_cave.id)) {
            if (Math.random() < 0.5) {
                const fragment = createItemInstance(ALL_ITEMS.i_key_fragment_cave.id);
                if (fragment) fragmentsObtainedThisBattle.push(fragment);
            }
        }
        // Tower Fragment from Dragonlord (after defeating the Dragonlord stage)
        else if (defeatedBossId === ALL_ENEMIES.e_micchy_dragonlord.id && !player.inventory.some(item => item.id === ALL_ITEMS.i_key_fragment_tower.id)) {
            if (Math.random() < 0.5) { 
                 const fragment = createItemInstance(ALL_ITEMS.i_key_fragment_tower.id);
                 if (fragment) fragmentsObtainedThisBattle.push(fragment);
            }
        }
    }
    ...
    if (isSfxEnabled) {
        if (win) playSfx(SFX_FILES.VICTORY);
        else playSfx(SFX_FILES.GAME_OVER);
    }

    if (win && isBikerBossFight) {
        setCurrentRun(prevRun => {
            if (!prevRun) return null;
            return {
                ...prevRun,
                ...
                player: runPlayerEndState, // Preserve HP/MP from Biker fight
            };
        });
        setModalMessage("宣伝バイカー「熱い･･カメラもカラダも熱暴走しそうだ､体が裂ける･･」");
        setBattleTransitionState('biker_defeated');
        return; 
    }
    ...
    setPlayer(prevPlayer => {
      ...
      const totalXpForRun = (isDragonlordBossFight || !isMitchyTower) ? (currentRun.xpGainedThisRun + xpGained) : currentRun.xpGainedThisRun;
      const totalGoldForRun = (isDragonlordBossFight || !isMitchyTower) ? (currentRun.goldGainedThisRun + goldGained) : currentRun.goldGainedThisRun;
      
      updatedPersistentPlayer.experience += totalXpForRun;
      updatedPersistentPlayer.gold += totalGoldForRun;
      ...
      if (fragmentsObtainedThisBattle.length > 0) {
        updatedPersistentPlayer.inventory = [...updatedPersistentPlayer.inventory, ...fragmentsObtainedThisBattle];
      }
    ...
    const newKeyFragmentMessage = fragmentsObtainedThisBattle.length > 0
        ? fragmentsObtainedThisBattle.map(f => `「${f.name}」をてにいれた！`).join('\n')
        : null;

    if (win) {
      const nextEncounterIndex = currentRun.currentEncounterIndex + 1;

      if (nextEncounterIndex > currentRegion.encounters.length || isDragonlordBossFight) { 
        ...
        if (currentRun.currentRegionId === REGIONS.r_micchy_castle.id) { 
             setGamePhase(GamePhase.ENDING_MESSAGE);
             return;
        }
        ...
        if (playerAfterBattleUpdate && clearedRegionKeyIndex !== -1 && clearedRegionKeyIndex + 1 < regionKeys.length) {
            ...
            if (regions[nextRegionIdToUnlock] && !regions[nextRegionIdToUnlock].isUnlocked) {
                ...
                if (nextRegionIdToUnlock === REGIONS.r_micchy_castle.id) {
                     ...
                     if (!allKeysPossessed) {
                        canUnlock = false;
                        const possessedKeyNames = playerAfterBattleUpdate.inventory.filter(item => item.isKeyItem && requiredKeyIds.includes(item.id)).map(item => item.name);
                        const missingKeyNames = requiredKeyIds
                            .filter(keyId => !playerAfterBattleUpdate.inventory.some(item => item.id === keyId))
                            .map(keyId => ALL_ITEMS[keyId]?.name || "なぞの破片");
                        
                        let keyMessage = "\nミッチー大魔王の元へ進むには、3つの鍵の破片が必要だ。\n";
                        if (missingKeyNames.length > 0) {
                            keyMessage += "たりない破片: " + missingKeyNames.join("、 ");
                        }
                        clearMessage += keyMessage; 
                     }
                }
                
                let messageForNextAreaUnlock = "";
                if (canUnlock) {
                    setRegions(prevRegions_1 => ({
                        ...prevRegions_1,
                        [nextRegionIdToUnlock]: { ...prevRegions_1[nextRegionIdToUnlock], isUnlocked: true }
                    }));
                    messageForNextAreaUnlock = `\nあたらしいちいき「${REGIONS[nextRegionIdToUnlock].name}」がかいほうされた！次はそこを目指してみよう！`;
                }
                clearMessage += messageForNextAreaUnlock;
            }
        }
        ...
        // This block is for when there are more encounters or the next one is the boss
        if (playerAfterBattleUpdate && nextEncounterIndex === currentRegion.encounters.length && currentRegion.bossId) {
            const bossToFaceNext = (isMitchyTower && currentRun.currentStageInBossFight === 2) 
                                    ? ALL_ENEMIES.e_micchy_dragonlord 
                                    : ALL_ENEMIES[currentRegion.bossId];
            
            const definedRegionFromConstants = REGIONS[currentRegion.id];
            const requiredLevelForNextBoss = definedRegionFromConstants?.bossUnlockLevel;


            if (typeof requiredLevelForNextBoss === 'number' && requiredLevelForNextBoss > 0 && playerAfterBattleUpdate.level < requiredLevelForNextBoss) {
                let blockMessage = `${currentRegion.name} のボス「${bossToFaceNext.name}」に挑戦するにはまだレベルが足りない (必要レベル: ${requiredLevelForNextBoss})。ワールドマップにもどります。`;
        ...
        setGamePhase(GamePhase.BATTLE_REWARD_SKILL_CARD); 
      }
    } else { 
      if (player) { 
  ...
  const handleModalClose = () => {
    if (battleTransitionState === 'biker_defeated') {
        setModalMessage(null); 
        setBattleTransitionState('none');
        setCurrentRun(prev => {
            if (!prev) return null;
            return {
                ...prev,
                currentStageInBossFight: 2,
            };
        });
        setGamePhase(GamePhase.BATTLE);
    } else {
  ...
  const handleProceedToCredits = () => {
    ...
    endingBgmAudioRef.current = new Audio(BGM_FILES.ENDING); // Use new ENDING BGM
  ...
  const renderScreen = () => {
    switch (gamePhase) {
      ...
      case GamePhase.NAME_INPUT:
        return <NameInputScreen setPlayerName={handleNameSet} setGamePhase={setGamePhase} />;
      ...
      case GamePhase.WORLD_MAP:
        if (!player) return <TitleScreen setGamePhase={setGamePhase} hasSaveData={false} startNewGame={startNewGameFlow} continueGame={continueGameFlow} onAudioStart={handleAudioEnable} onPlayTitleMusicRequest={handlePlayTitleMusicRequest} isTitleMusicPlaying={isTitleMusicPlaying} />; 
        return <WorldMapScreen regions={regions} onSelectRegion={handleAttemptRegionSelect} setGamePhase={setGamePhase} player={player}/>;
      case GamePhase.BATTLE:
        ...
        const enemiesForBattle = getEnemiesForEncounter(currentRegionForBattle, currentRun.currentEncounterIndex, currentRun.currentStageInBossFight);
        ...
        return <BattleScreen
                  key={currentRun.currentRegionId + '-' + currentRun.currentEncounterIndex + '-' + (currentRun.currentStageInBossFight || '1')}
      ...
      case GamePhase.STATUS_SCREEN:
        ...
        return <StatusScreen player={player} setGamePhase={setGamePhase} onEquipItem={handleEquipItem} onUnequipItem={handleUnequipItem} onEnhanceEquipment={handleEnhanceEquipment}/>;
      case GamePhase.SHOP:
        if (!player) return <p>プレイヤーデータがありません。</p>;
        return <ShopScreen player={player} currentShopRegionId={currentShopRegionId} allRegions={regions} onPurchaseItem={handlePurchaseItem} onSellItem={handleSellItem} setGamePhase={setGamePhase} />;
      case GamePhase.GACHA:
        if (!player) return <p>プレイヤーデータがありません。</p>;
        return <GachaScreen player={player} currentGachaRegionId={currentGachaRegionId} allRegions={regions} onUseGacha={handleUseGacha} setGamePhase={setGamePhase} />;
      case GamePhase.WISDOM_BAG: 
        if (!player) return <p>プレイヤーデータがありません。</p>;
        return <WisdomBagScreen player={player} setGamePhase={setGamePhase} />;
      ...
      case GamePhase.PASSWORD_SAVE:
        return <PasswordManagementScreen mode="save" playerData={player} regionsData={regions} setGamePhase={setGamePhase} onLoadSuccess={handlePasswordLoadSuccess} />;
      case GamePhase.PASSWORD_LOAD:
        return <PasswordManagementScreen mode="load" playerData={player} regionsData={regions} setGamePhase={setGamePhase} onLoadSuccess={handlePasswordLoadSuccess} />;
      default:
  ...
  return (
    <div className="w-full h-full flex flex-col items-stretch justify-stretch overflow-hidden">
      {renderScreen()}
      {modalMessage && (
        <Modal isOpen={!!modalMessage} onClose={handleModalClose} title="おしらせ">
